<template>
  <div class="invoice-index-page invoices main-content">
    <div class="page-header">
      <h3 class="page-title">{{ $t('Booking') }}</h3>
      <ol class="breadcrumb">
...
export default {
  ...
  methods: {
    ...
    async fetchData ({ page, filter, sort }) {
      ...
      let response = await this.fetchBookings(data)
      this.isRequestOngoing = false

      this.currency = response.data.currency

      return {
        data: response.data.bookings.data,
        pagination: {
          totalPages: response.data.bookings.last_page,
          currentPage: page,
          count: response.data.bookings.count
        }
      }
    },
    setFilters () {
